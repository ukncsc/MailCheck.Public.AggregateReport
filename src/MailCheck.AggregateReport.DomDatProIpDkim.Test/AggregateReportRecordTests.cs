...
namespace MailCheck.AggregateReport.DomDatProIpDkim.Test
{
    ...
    public class AggregateReportRecordTests
    {
        ...
        {
            AggregateReportRecordEnriched aggregateReportRecordEnriched = CreateTestRecord(new List<string> { "domain1:selector1:pass" }, spfResult, dkimResult, blockListCount: blocklistCount);
            ...
            int blockListCount = 0, string hostSourceIp = "192.168.1.1", string hostProvider = "mail.host.provider")
        {
            return new AggregateReportRecordEnriched("id", "1", "correlationId", "causationId", "orgName", "reportId",
                ...
                new List<string>(){$"{headerFrom}:{dkimResult}"}, 1, 1, dkimAuthResults, 1, 1, true, true, true, true, true, true, true, "hostName", "hostOrganisationDomain", hostProvider, 1,
