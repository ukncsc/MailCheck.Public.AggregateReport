    ...
    public class AggregateReportRecordTests
    {
            ...
            int blockListCount = 0)
        {
            return new AggregateReportRecordEnriched("id", "1", "correlationId", "causationId", "orgName", "reportId",
                ...
                count, disposition, dkimResult, spfResult, "envelopeTo", "envelopeFrom", headerFrom,  organisationDomain, new List<string>(){$"{headerFrom}:{spfResult}"},1, 1, new List<string>(){$"{headerFrom}:{dkimResult}"}, 1, 1, true,
                true, true, true, true, true, true, "hostName", "hostOrganisationDomain", "hostProvider", 1, "asDescription", "country",
