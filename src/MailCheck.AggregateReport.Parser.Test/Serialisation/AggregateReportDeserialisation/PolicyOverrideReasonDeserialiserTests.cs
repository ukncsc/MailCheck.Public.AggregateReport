...
namespace MailCheck.AggregateReport.Parser.Test.Serialisation.AggregateReportDeserialisation
{
    ...
    public class PolicyOverrideReasonDeserialiserTests
    {
        ...
        [Test]
        public void ArcCommentGeneratesArcOverrideReason()
        {
            XElement xElement = XElement.Parse(PolicyOverrideReasonDeserialserTestsResources.PolicyOverrideReasonArc);
            PolicyOverrideReason[] policyOverrideReasons = _policyOverrideReasonDeserialiser.Deserialise(new[] { xElement });

            Assert.That(policyOverrideReasons.First().PolicyOverride, Is.EqualTo(PolicyOverride.arc));
            Assert.That(policyOverrideReasons.First().Comment, Is.EqualTo("arc=pass"));
        }
