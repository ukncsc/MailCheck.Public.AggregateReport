...
{
    internal class SqlLitePersistor : IDenormalisedRecordPersistor
    {
        ...
        {
            CreateDirectoryAndRemoveOldFiles(_commandLineArgs.SqlFile);

            using (var connection = new SqliteConnection($"DataSource={_commandLineArgs.SqlFile.FullName};Cache=Shared"))
            {
                connection.Open();
                ...
                {
                    foreach (DenormalisedRecord record in records)
                    {
                        ...
                        CreateParameters(command, record);
